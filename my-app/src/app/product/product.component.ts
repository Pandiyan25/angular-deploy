...
export class ProductComponent {
  ...
  getRating() {
    return this.products.filter((product) => product.rating > 4.5).length;
  }

  getOffers() {
    return this.products.filter((product) => product.discountPercentage >= 15.0)
      .length;
  }
  courseRadio: string = 'all';

  onfilterRadioChanged(data: string) {
    this.courseRadio = data;
  }
  searchedValue: string = '';

  findProduct(data: string) {
    this.searchedValue = data;
    console.log(this.searchedValue, 'asd');
  }

  filteredProducts() {
    let filtered = this.products;

    if (this.courseRadio === 'ratings') {
      filtered = filtered.filter((product) => product.rating > 4.5);
    } else if (this.courseRadio === 'offers') {
      filtered = filtered.filter(
        (product) => product.discountPercentage >= 15.0
      );
    }

    if (this.searchedValue !== '') {
      const searchTerm = this.searchedValue.toLowerCase();
